...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!member) return message.channel.send(embed.setDescription("Eksik argüman: Kullanıcı etiketlemediniz."))
        if (member.roles.highest.position >= message.member.roles.highest.position) {
            return message.channel.send(embed.setDescription("Hata kod #003 : Belirttiğin kullanıcı seninle aynı ve/veya/ya da senden üst bir pozisyonda!"))
        }
