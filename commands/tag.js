...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!client.config.tagrolcü.some(id => message.member.roles.cache.has(id))) {
        ...
        await message.guild.members.cache.get(member.id).roles.add(client.config.tagrol)
        message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setDescription(`${member} adlı kullanıcıya <@&851765314110226432> rolü verildi`).setColor('#ffdd06').setTimestamp().setThumbnail(message.author.avatarURL).setFooter('Kullanıcı .e/.k komutları ile kayıt edilebilir!'))
         message.react('<a:loveu:843792337813110794>')        
    }
}